...
for(const p of coverage.provinces){
 ...
 checks.push({province:p.name,locality,ean:product.id,stores:prices.body.data.sucursales.length,pricesMatchSource:true,officialLocality:true,within50Km:true,durationMs:Date.now()-started});console.log(p.name+': búsqueda, sucursal y precio comprobados');
 if(p.code==='AR-J'){
