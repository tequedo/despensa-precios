...
const root=process.env.NATIONAL_OUTPUT_DIR??'data/national',coverage=JSON.parse(await readFile(join(root,'coverage.json'),'utf8')),rejected=[];
...
await writeFile(process.env.GEOGRAPHY_AUDIT_FILE??'data/national-geography-audit.json',JSON.stringify({checkedAt:coverage.geographyCheckedAt,source,rejectedCount:rejected.length,rejected},null,2)+'\n');
